...
const replaceBlock = (str, startStr, endStr, newBlock) => {
    ...
    if (startIndex === -1) {
        console.error('Failed to find:', startStr.substring(0, 50));
        return str;
    }
    let endIndex = str.indexOf(endStr, startIndex);
    if (endIndex === -1) {
        console.error('Failed to find end:', endStr.substring(0, 50));
        return str;
    }
...
const replaceAllBlocks = (str, startStr, endStr, newBlock) => {
    let modified = str;
    while (modified.indexOf(startStr) !== -1) {
        let nextStr = replaceBlock(modified, startStr, endStr, newBlock);
        if (nextStr === modified) break; // prevent infinite loop if somehow not replaced
        modified = nextStr;
    }
    return modified;
};
...
// Normalise line endings for search
let normalizedFile = file.replace(/\\r\\n/g, '\\n');

let lastModified = normalizedFile;
lastModified = replaceBlock(lastModified, addTeamStart, addTeamEnd, addTeamReplace);
lastModified = replaceBlock(lastModified, addSupStart, addSupEnd, addSupReplace);

lastModified = replaceAllBlocks(lastModified, eqSupClickStart1, eqSupClickEnd1, eqSupClickRep1);
lastModified = replaceAllBlocks(lastModified, eqTeamClickStart2, eqTeamClickEnd2, eqTeamClickRep2);

// Add helpers
lastModified = lastModified.replace('export const CallTrackingTable: React.FC<CallTrackingTableProps> = ({', helpers);

// Cleanup duplicate imports
lastModified = lastModified.replace('  DropdownItem,\\n  DropdownItem,', '  DropdownItem,');

// Restore original line endings if needed, but modern tools handle \n just fine.
fs.writeFileSync('src/components/dispatch/calltracking.tsx', lastModified);
