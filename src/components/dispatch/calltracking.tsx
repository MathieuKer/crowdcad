...
const EquipmentSubmenuBuilder = ({ event, call, equipment, callDisplayNumberMap, updateEvent, isClinic }: { event: Event, call: Call, equipment: Equipment, callDisplayNumberMap: Map<string, number>, updateEvent: (updates: Partial<Event>) => Promise<void>, isClinic: boolean }) => {
  return (
    <DropdownMenuSub>
      <DropdownMenuSubTrigger className={`text-surface-light hover:bg-surface-liner focus:bg-surface-liner cursor-pointer ${isClinic ? 'bg-status-blue/20' : ''}`}>
        {equipment.name}
      </DropdownMenuSubTrigger>
      <DropdownMenuSubContent className="bg-surface-deep border-surface-liner">
        {/* Teams */}
        {event.staff?.filter((team: Staff) => !call.assignedTeam?.includes(team.team) && !isTeamOnActiveCall(event.calls, team.team) && team.status === 'Available').map((team: Staff) => (
          <DropdownMenuItem
            key={`equip-team-${isClinic ? 'clinic-' : ''}${team.team}`}
        ...
        {event.staff?.filter((team: Staff) => !call.assignedTeam?.includes(team.team) && !isTeamOnActiveCall(event.calls, team.team) && ['In Clinic', 'On Break'].includes(team.status)).map((team: Staff) => (
          <DropdownMenuItem
            key={`equip-team-inactive-${isClinic ? 'clinic-' : ''}${team.team}`}
        ...
        {/* Supervisors */}
        {event.supervisor?.filter((sup: Supervisor) => !call.assignedTeam?.includes(sup.team) && !isTeamOnActiveCall(event.calls, sup.team) && sup.status === 'Available').map((supervisor: Supervisor) => (
          <DropdownMenuItem
            key={`equip-supervisor-${isClinic ? 'clinic-' : ''}${supervisor.team}`}
            className="text-surface-light hover:bg-surface-liner focus:bg-surface-liner cursor-pointer"
        ...
        {event.supervisor?.filter((sup: Supervisor) => !call.assignedTeam?.includes(sup.team) && !isTeamOnActiveCall(event.calls, sup.team) && ['In Clinic', 'On Break'].includes(sup.status)).map((supervisor: Supervisor) => (
          <DropdownMenuItem
            key={`equip-supervisor-inactive-${isClinic ? 'clinic-' : ''}${supervisor.team}`}
            className="text-surface-light hover:bg-surface-liner focus:bg-surface-liner cursor-pointer bg-status-blue/20"
            onClick={() => assignEquipmentToCall(event, call, equipment, supervisor.team, true, callDisplayNumberMap, updateEvent)}
          >
            {supervisor.team}
                    ...
                    >
                      ...
                      >
                        {editingCell?.callId === call.id && editingCell.field === 'chiefComplaint' ? (
                          <input
                            ...
                            onBlur={async () => {
                              ...
                              try {
                                ...
                                console.error('Failed to update chiefComplaint', err);
                          ...
                          ((pendingForCall.chiefComplaint ?? call.chiefComplaint) || <span className="text-surface-light whitespace-nowrap">[Edit]</span>)
                        )}
                      </td>
                          ...
                          <input
                            ...
                            onBlur={async () => {
                              ...
                              try {
                                ...
                                console.error('Failed to update ageSex', err);
                              }
                            }}
                            onFocus={(e) => {
                                  ...
                                  {(() => {
                                    ...
                                    return [
                                      ...
                                      ...availableEquipment.map((equipment: Equipment) => (
                                        <EquipmentSubmenuBuilder
                                          key={`avail-${equipment.id}`}
                                          event={event}
                                          call={call}
                                          equipment={equipment}
