...
import { Event, Call, InteractionSession } from '@/app/types';
import dynamic from 'next/dynamic';
import { Button, Card, CardBody } from '@heroui/react';
import { DiagonalStreaks } from '@/components/ui/diagonal-streaks';
import { downloadEventCSV, downloadDataCollectionCSV, formatCsvTimestamp as formatTimestamp } from '@/utils/csvExport';
...
function getScheduleWindow(event: Event): { start: number; end: number } {
  ...
  const starts = startFields.map(k => getNum(event[k as keyof Event])).filter(Boolean) as number[];
  const ends = endFields.map(k => getNum(event[k as keyof Event])).filter(Boolean) as number[];

  const { minTs, maxTs } = event.calls?.reduce((acc, call) => {
    call.log?.forEach(entry => {
      if (typeof entry.timestamp === 'number') {
        if (entry.timestamp < acc.minTs) acc.minTs = entry.timestamp;
        if (entry.timestamp > acc.maxTs) acc.maxTs = entry.timestamp;
      }
    });
    return acc;
  }, { minTs: Number.POSITIVE_INFINITY, maxTs: Number.NEGATIVE_INFINITY }) || { minTs: Number.POSITIVE_INFINITY, maxTs: Number.NEGATIVE_INFINITY };
...
export default function SummaryPage() {
  ...
  const handleCSVDownload = () => downloadEventCSV(event);
  const handleDataCollectionCSVDownload = () => downloadDataCollectionCSV(event, interactionSessions);
  ...
  if (event?.calls) {
    event.calls.forEach((call) => {
      ...
      calculateTeamDurations(call, involvedTeams, teamCallDurations);
    });
  }

  function calculateTeamDurations(call: Call, involvedTeams: string[], durations: Record<string, number>) {
    for (const team of involvedTeams) {
      ...
      if (start != null && end != null && end > start) {
        durations[team] = (durations[team] || 0) + (end - start);
      }
    }
  }
